...
$parentModule = 'Get-ModulePaths'
if (-not $ModuleName) { $ModuleName = 'Get-ModulePaths.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\Get-ModulePaths'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\Get-ModulePaths.psd1"
$prereqHash = @{
