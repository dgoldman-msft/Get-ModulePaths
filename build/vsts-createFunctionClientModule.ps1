...
$parentModule = 'GetModulePath'
if (-not $ModuleName) { $ModuleName = 'GetModulePath.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\GetModulePath'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\GetModulePath.psd1"
$prereqHash = @{
